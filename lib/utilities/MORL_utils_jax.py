...
def eval_agent(test_env, w_batch, get_action, actor_state, args, key, eval_episodes=1):
    ...
    for eval_ep in range(eval_episodes):
        ...
        for evalPreference in w_batch:
            ...
            while not terminal:
                preference_ep, action, key = get_action(deterministic = True, actor_state =actor_state, states = state, preference = evalPreference, key = key)
    ...
    return hypervolume.mean(), sparsity.mean(), objs.mean(axis=0), key
...
def eval_agent_interp(test_env, w_batch, get_action, actor_state, args, key, eval_episodes=1):
